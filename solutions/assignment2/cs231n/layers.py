...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    xhat = (x - sample_mean) / std_dev

    out = gamma * xhat + beta

    cache = (x, gamma, sample_mean, xhat, std_dev)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
...
def batchnorm_backward(dout, cache):
  ...
  x, gamma, sample_mean, xhat, std_dev = cache

  #############################################################################
  # didn't end up doing computation graph, just used the quotient rule:
  #
  #      [f(x)]'   g(x)f'(x) - f(x)g'(x)
  # dx = |----|  = ---------------------
  #      [g(x)]           [g(x)]^2
  #
  #
  # where f(x) = x - E[X] and g(x) = sqrt(Var[X])
  #
  # Note a single example x[i] affects every output y through the expectation
  ...
  #     dgx = xhat[m,d] / N
  #     for k in range(N):
  #       fx = x[k,d] - sample_mean[d]
  #       if k == m:
  #         dfx = 1 - 1./N
  #       else:
  #         dfx = - 1./N
  #
  #       dx[m,d] += dout[k,d] * (gx*dfx - fx*dgx)/(gx**2)
  #
  # dx = gamma * dx

  #this vectorization splits the above formula into 2 separate terms, g*f'/g^2 - f*g'/g^2 = f'/g - f*g'/g^2
  Gx = std_dev
  dGx = xhat/N
  Fx = x - sample_mean

  #first term
  dX11 = np.sum(dout * (-1./N)/Gx, axis=0) #portion of expectation derivative that doesn't change per row: -1/N
  dX12 = dout * 1/Gx                       #portion of expectation derivative that does change per row: 1 when k == m, zero otherwise (see iterative answer)

  #second term
  dX2 = np.sum(dout * Fx, axis=0) * dGx / Gx / Gx

  dx = gamma * (dX11 + dX12 - dX2)
...
def batchnorm_backward_alt(dout, cache):
  ...
  #############################################################################
  # a fully simplified implementation (see tex doc)
  #############################################################################
  ...
  #       if k == m:
  #         dx_minus_ninv += 1
  #       sum1 += dout[k,d]*xhat[k,d]
  #       sum2 += dout[k,d]
  #     sum1 = xhat[m,d]*sum1/N
  #     sum2 = sum2/N
  #     dx[m,d] = (dout[m,d] - sum1 - sum2)/sd
  # dx = gamma * dx


  dX = (dout - np.sum(dout*xhat, axis=0)*xhat/N - np.mean(dout, axis=0)) * gamma / std_dev
  dgamma = np.sum(xhat*dout, axis=0)
  dbeta = np.sum(dout, axis=0)
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def avgnorm_forward(x, gamma, beta):
  """
  Input:
  - x: Data of shape (N, D)
  - gamma: Scale parameter of shape (D,)
  ...
  out = np.tile(sample_mean, (x.shape[0], 1))
  out = gamma * out + beta
  cache = (gamma, sample_mean)

  return out, cache


def avgnorm_backward(dout, cache):
  """
  Inputs:
  - dout: Upstream derivatives, of shape (N, D)
  - cache: Variable of intermediates from batchnorm_forward.
  ...
  gamma, sample_mean = cache

  m = dout.shape[0]
  dx = gamma * np.sum(dout / m, axis=0)
  dx = np.tile(dx, (dout.shape[0], 1))
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def batchnorm_num_forward(x, gamma, beta):
  """
  Input:
  - x: Data of shape (N, D)
  - gamma: Scale parameter of shape (D,)
  ...
  out = x - np.tile(sample_mean, (x.shape[0], 1))
  out = gamma * out + beta
  cache = (gamma, sample_mean)
...
def batchnorm_num_backward(dout, cache):
  """
  Inputs:
  - dout: Upstream derivatives, of shape (N, D)
  - cache: Variable of intermediates from batchnorm_forward.
...
def batchvar_forward(x, gamma, beta):
  """
  Input:
  - x: Data of shape (N, D)
  - gamma: Scale parameter of shape (D,)
  ...
  out, cache = None, None
  sample_var = np.var(x, axis=0)
  out = np.tile(sample_var, (x.shape[0], 1))
  out = gamma * out + beta
  cache = (gamma, x)
...
#   dx_i           dy_k   dx_i
#
# Note the variance derivative is a sum and then we have to sum up across all examples. this
# is what leads to the double summation
#
def batchvar_backward(dout, cache):
  """
  Inputs:
  - dout: Upstream derivatives, of shape (N, D)
  - cache: Variable of intermediates from batchnorm_forward.
  ...
  dx = np.sum(dout, axis=0) * gamma * (2./n**2) * (t1 - t2)

  return dx


def batchnorm_den_forward(x, gamma, beta):
  """
  Input:
  - x: Data of shape (N, D)
  - gamma: Scale parameter of shape (D,)
  ...
  out, cache = None, None
  sample_var = np.var(x, axis=0)
  std_dev = np.sqrt(sample_var + 1e-5)
  out = np.tile(std_dev, (x.shape[0], 1))
  out = gamma * out + beta
  cache = (gamma, sample_var, std_dev, x)

  return out, cache


def batchnorm_den_backward(dout, cache):
  """
  Inputs:
  - dout: Upstream derivatives, of shape (N, D)
  - cache: Variable of intermediates from batchnorm_forward.
  ...
  gamma, var, std_dev, x = cache

  n = x.shape[0]
  t1 = n * x
  t2 = np.sum(x, axis=0)

  dx = np.sum(dout, axis=0) * gamma * .5/std_dev * (2./n**2) * (t1 - t2)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
